...
K=2        # dimension of latent feature space
MAXITER=10 # maximum number of iterations
THREADS=16 # number of threads


echo "Alternate least square (ALS) on "$TRAINING
./als --matrixmarket=true --training=$TRAINING --D=$K --maxiter=$MAXITER --ncpus=$THREADS --quiet=1
echo "Stochastic Gradient Descent (SGD) on "$TRAINING
./sgd --matrixmarket=true --training=$TRAINING --D=$K --maxiter=$MAXITER --ncpus=$THREADS --quiet=1
...
echo "Alternate least square (ALS) on "$TRAINING
./als --matrixmarket=true --training=$TRAINING --validation=$VALIDATION --D=$K --maxiter=$MAXITER --ncpus=$THREADS --minval=1 --maxval=5 --quiet=1
echo "Stochastic Gradient Descent (SGD) on "$TRAINING
./sgd --matrixmarket=true --training=$TRAINING --validation=$VALIDATION --D=$K --maxiter=$MAXITER --ncpus=$THREADS --minval=1 --maxval=5 --quiet=1
