...
convert_fields = {
        ...
        'date': lambda r: datetime.strptime(r['date'], '%Y-%m-%d'),
...
with open('yelp_phoenix_academic_dataset/yelp_academic_dataset_review.json', 'rb') as rev_file:
    reviews = map(json.loads, rev_file)

print "loading users"
with open('yelp_phoenix_academic_dataset/yelp_academic_dataset_user.json', 'rb') as rev_file:
    users = map(json.loads, rev_file)
user_by_id = dict((u['user_id'], u) for u in users)

print "loading businesses"
with open('yelp_phoenix_academic_dataset/yelp_academic_dataset_business.json', 'rb') as rev_file:
