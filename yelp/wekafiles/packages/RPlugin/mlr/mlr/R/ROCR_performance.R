...
ROCR.performance <- function(prediction.obj, measure,
    ...
    if (class(prediction.obj) != 'ROCR.prediction' ||
      ...
      stop(paste("Wrong argument types: First argument must be of type",
                 "'ROCR.prediction'; second and optional third argument must",
    ...
    if (x.measure == "cutoff" ||
        ...
        perf.obj.1 <- ROCR.performance( prediction.obj, measure=x.measure, ... )
        perf.obj.2 <- ROCR.performance( prediction.obj, measure=measure, ... )
