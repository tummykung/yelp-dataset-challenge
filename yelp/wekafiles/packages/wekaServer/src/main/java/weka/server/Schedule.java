...
public class Schedule implements Serializable, OptionHandler {
  ...
  public Enumeration<Option> listOptions() {
    ...
    newVector.add(new Option("\tSet the start date (e.g. -start 2011-08-28T13:45)",
        "start", 1 , "-start <date>"));
    newVector.add(new Option("\tSet the end date (e.g. -start 2011-08-31T9:30)",
        "end", 1 , "-end <date>"));
    newVector.add(new Option("\tSet the repeat unit (e.g. once, minutes, hours, " +
    		"days, weekly, monthly or yearly)", "repeat-unit", 
    		1, "-repeat-unit <unit>"));
    ...
    newVector.add(new Option("\tSet the day of the week (weekly and monthly " +
    		"repeat units only. E.g mon, tue etc.)", "day-of-week", 
    		1, "-day-of-week <day>"));
    newVector.add(new Option("\tSet the month of the year (yearly " +
        "repeat units only. E.g. jan, feb, etc.)", 
        "month-of-year", 1, "-month-of-year <month>"));
    newVector.add(new Option("\tSet the day of the month (monthly repeat units " +
    		"only. Execute on this day of every month. Range 1 - 31)", 
    		"day-of-month",1, "-day-of-month <day number>"));
    newVector.add(new Option("\tSet occurrence within month. Use in conjunction " +
  ...
  public String[] getOptions() {
    ...
    if (m_dayOfTheWeek.size() > 0) {
      ...
      for (Integer dayOfTheWeek : m_dayOfTheWeek) {
        if (dayOfTheWeek >= Calendar.SUNDAY && 
            ...
            (m_repeat == Repeat.WEEKLY ||
                m_repeat == Repeat.MONTHLY ||
                m_repeat == Repeat.YEARLY)) {
    ...
    if (m_monthOfTheYear >= Calendar.JANUARY && m_monthOfTheYear <= Calendar.DECEMBER &&
        m_repeat == Repeat.YEARLY) {
    ...
    if (m_dayOfTheMonth > 0 && m_dayOfTheMonth <= 31 
        && m_occurrence == null && m_repeat == Repeat.MONTHLY) {
  ...
  public void setOptions(String[] options) throws Exception {
    ...
    if (configCheck.length() > 0) {
      throw new Exception("Problem with configuration of Schedule: " + configCheck);
    }
  }
  ...
  public String checkConfig() {
    ...
    case MINUTES:
      if (m_repeatValue <= 0 || m_repeatValue > 60) {
        return "Repeat of " + m_repeatValue + " is out of range for MINUTES repeat unit!";
      }
      return "";
    case HOURS:
      if (m_repeatValue <= 0 || m_repeatValue > 60) {
        return "Repeat of " + m_repeatValue + " is out of range for HOURS repeat unit!";
      }
      return "";
    case DAYS:
      if (m_repeatValue <= 0) {
  ...
  /**
   * Get the next execution with respect to this schedule and the supplied
   * date of the last execution (may be null if there has been no previous
   * execution).
   * 
   * @param lastExecution date of the last execution or null (if no previous execution)
...
/*          if (cal.get(Calendar.DAY_OF_WEEK) > m_dayOfTheWeek) {
          ...
          cal.set(Calendar.DAY_OF_WEEK, m_dayOfTheWeek); */
        }
    ...
    if (m_repeat == Repeat.MONTHLY) {
      if (m_occurrence == null) {
        if (lastExecution == null) {
          ...
          if (m_start != null && cal.getTime().before(m_start)) {
            // can't really happen if we've already executed at least once previously...
            return null;
          }
          if (m_end != null && cal.getTime().after(m_end)) {
            return null;
        ...
        if (lastExecution == null) {
          ...
          d = new Date(lastExecution.getTime()); // should have the correct hour and min
          GregorianCalendar temp = new GregorianCalendar();
          temp.setTime(d);
    ...
    if (m_repeat == Repeat.YEARLY) {
      ...
      GregorianCalendar cal = new GregorianCalendar();
      cal.setTime(new Date(lastExecution.getTime())); // should have correct hour and min
      cal.add(Calendar.YEAR, 1);
      cal.set(Calendar.MONTH, m_monthOfTheYear);
      cal.set(Calendar.DAY_OF_MONTH, 1);
      cal = setRequestedDayInMonth(cal);
  ...
  private GregorianCalendar setRequestedDayInMonth(GregorianCalendar cal) {
    ...
    boolean done = false;
    while (cal.get(Calendar.DAY_OF_MONTH) <= 31 && cal.get(Calendar.MONTH) == month) {
  ...
  public boolean execute(Date lastExecution) 
    throws Exception {
  ...
  private boolean checkYearly(Date now, Date lastExecution) {
    ...
    if (nowMonth == m_monthOfTheYear) {
      ...
      // now does the particular day in the month (i.e. last wednesday of month) match
      return checkMonthly(now, lastExecution);
    }
  ...
  private boolean checkMonthly(Date now, Date lastExecution) {
    ...
    if (m_occurrence == null) {
      ...
      // specific occurrence of a particular day in the month (e.g. second tuesday)
