  ...
  throws ServletException, IOException {
    ...
    try {
      ...
      if (found != null) {
        ...
        theTask.getTaskStatus().setExecutionStatus(Integer.parseInt(taskStatus));
        
        // pass on to our master?
        if (found.getCameFromMaster()) {
          m_server.sendTaskStatusInfoToMaster(found, 
              theTask.getTaskStatus().getExecutionStatus());
        }
        ...
        System.err.println("[WekaServer] Received a task status update from a slave, " +
            "but was unable to find task with remote ID '" 
            + taskName + "' in the list of tasks registered with this server.");
        String errorResult = WekaServlet.RESPONSE_ERROR + ": Master received task status " +
        "update but couldn't find task with remote ID '" + taskName + "' in its " +
