...
public class AddSlaveServlet extends WekaServlet {
  ...
  public void doGet(HttpServletRequest request, HttpServletResponse response)
    ...
    try {
      if (client) {
        if (!ok) {
          String errorResult = WekaServlet.RESPONSE_ERROR + ": malformed host address (need host:port)" +
          " - " + slaveToAdd;
        ...
        if (!ok) {
          ...
          out.println(WekaServlet.RESPONSE_OK + ": Slave '" 
              + slaveToAdd + "' added successfully. </H3>");
          out.println("<a href=\"" + RootServlet.CONTEXT_PATH + "\">" 
              + "Back to status page</a></br>");
          out.println("</BODY>\n</HTML>");
        }
