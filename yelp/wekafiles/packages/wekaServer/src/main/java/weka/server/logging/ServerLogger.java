...
/**
 * Class that provides logging to a file and also maintains
 * the most recent 100 log lines and 50 status lines in memory.
 * 
...
public class ServerLogger implements Logger {
  ...
  protected static final String SERVER_LOG_DIRECTORY = 
    WekaServer.SERVER_ROOT_DIRECTORY + "logs" + File.separator;
  ...
  static {
    ...
    if (!logDir.exists()) {
      if (!logDir.mkdir()) {
        System.err.println("Unable to create server logging " +
        		"directory (" + SERVER_LOG_DIRECTORY + ")");
   ...
   * 
   * @param taskEntry the task entry for the task using this
   * logger
   */
  public ServerLogger(WekaTaskMap.WekaTaskEntry taskEntry) {
    ...
    m_logSubDir = SERVER_LOG_DIRECTORY + taskEntry.getOriginatingServer().replace(":", "_") 
      + File.separator;
  ...
  protected synchronized void logMessage(String message, boolean fromFile) {
    ...
    if (!fromFile) {
      formatted = m_dateFormat.format(new Date()) + ": " + 
      m_taskNameID + " - " + message;
  ...
  /**
   * Load the most recent log and status messages from the log file
