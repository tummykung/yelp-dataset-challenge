...
public class AdvancedConfigPanel extends JPanel {
  ...
  protected AttributeSelectionPanelExtended m_dateDerivedPeriodicSelector = new AttributeSelectionPanelExtended(
  ...
  protected AttributeSelectionPanelExtended m_overlaySelector = new AttributeSelectionPanelExtended(
      true, true, true, true);
  ...
  protected AttributeSelectionPanel m_evaluationMetrics = new AttributeSelectionPanel(
      false, false, false, false);
  ...
  private Instances createAvailableOverlayList() {
    ...
    if (m_simpleConfig.m_targetHeader != null) {
      ...
      int[] selectedTargets = m_simpleConfig.m_targetPanel
      ...
      for (int i = 0; i < m_instances.numAttributes(); i++) {
        ...
        if (m_simpleConfig.m_targetHeader.attribute(m_instances.attribute(i)
          ...
          int indexToCheck = m_simpleConfig.m_targetHeader.attribute(
  ...
  protected void updateDateDerivedPanel() {
    ...
    String selectedTimeStamp = m_simpleConfig.m_timeStampCombo
  ...
  protected void updateEvalAndOutputEnabledStatus() {
    boolean enable = (m_trainingCheckBox.isSelected()
        || m_holdoutCheckBox.isSelected() || m_separateTestSetCheckBox
        .isSelected());
  ...
  protected void savePeriodicsToFile() throws IOException {
    if (m_fileChooser == null) {
      ...
      FileFilter filter = new ExtensionFileFilter(".periodics",
          "Time series date-derived "
    ...
    if (result == JFileChooser.APPROVE_OPTION) {
      ...
      PrintWriter br = new PrintWriter(new BufferedWriter(new FileWriter(
          saveFile)));
      ...
      if (m_dateDerivedPeriodicsHeader != null) {
        for (int i = 0; i < selected.length; i++) {
          ...
          if (s < NUM_PREDEFINED_PERIODICS) {
            ...
            ArrayList<CustomPeriodicTest> selectedP = m_customPeriodics
                .get(name);
    ...
    if (result == JFileChooser.APPROVE_OPTION) {
      ...
      while ((line = br.readLine()) != null) {
        if (line.startsWith("*pre-defined*:")) {
          ...
          currentCustomName = line.substring(line.indexOf(":") + 1,
              line.length()).trim();
          withinCustom = true;
  ...
  protected void layoutDateDerivedPeriodicPanel() {
    ...
    m_editCustomPeriodicBut.addActionListener(new ActionListener() {
      public void actionPerformed(ActionEvent e) {
        int selectedRow = m_dateDerivedPeriodicSelector.getTable()
            .getSelectedRow();
        if (selectedRow > NUM_PREDEFINED_PERIODICS - 1) {
          String fieldName = m_dateDerivedPeriodicsHeader
              .attribute(selectedRow).name();

          // need to trim off the "c_"
          fieldName = fieldName.replaceFirst("c_", "");
          ArrayList<CustomPeriodicTest> toEdit = m_customPeriodics
              .get(fieldName);
          if (toEdit == null) {
            ...
            try {
              ...
              if (newFieldName.length() == 0) {
                ...
                int[] selected = m_dateDerivedPeriodicSelector
                    .getSelectedAttributes();
                Instances insts = createDateDerivedPeriodicList();
                m_dateDerivedPeriodicSelector.setInstances(insts);
                m_dateDerivedPeriodicsHeader = insts;
  ...
  protected void layoutLagPanel() {
    ...
    final JLabel fineTuneLab = new JLabel("Fine tune lag selection",
        JLabel.RIGHT);
    ...
    final JLabel numConsLab = new JLabel("# consecutive lags to average",
        JLabel.RIGHT);
      ...
      if (m_useCustomLags.isSelected()) {
        ...
        if (m_averageLongLags.isSelected()) {
          ...
          int avLagsAfter = ((SpinnerNumberModel) m_averageLagsAfter.getModel())
              .getNumber().intValue();
          int numToAv = ((SpinnerNumberModel) m_numConsecutiveToAverage
